...
| **Routines CRUD + cadence-shift** (§50 Phase E — simplified `Routine` schema (dropped energy / flex_days / also_triggers / skip_if / budget_gated / cost_estimate); `patchRoutine` accepts a `cadence_shift_strategy` ∈ `one_off` / `shift_all` / `skip_one`; Stuff → Routines table lets her edit `interval_days` and prompts for a strategy on appointment-enabled cadence changes. `monthly_occurrences_override` lets `listOutsourceable`'s cost math match real-world booking cadence when the interval-based default is wrong) | [apps/api/src/db/models/Routine.ts](apps/api/src/db/models/Routine.ts), [apps/api/src/services/routines.ts](apps/api/src/services/routines.ts), [apps/dashboard/src/components/RoutinesPage.tsx](apps/dashboard/src/components/RoutinesPage.tsx) |
| **Per-month projected income** (§50 Phase E — `FinancialProfile.monthly_projected_income_overrides` Map<YYYY-MM, dollars> with per-month set/clear, falls back to `monthly_gross_income` when no override exists; new assistant tool `set_projected_income`; ProjectedIncomeEditor on Stuff → Finance; Look Back's This Month picks up the override automatically) | [apps/api/src/services/finance.ts](apps/api/src/services/finance.ts), [apps/dashboard/src/components/FinancePanel.tsx](apps/dashboard/src/components/FinancePanel.tsx) |
...
- **Projected income overrides** (§50 Phase E) — `getProjectedIncomeForMonth` override-vs-gross-fallback resolution + null-when-neither + malformed-month rejection; `setProjectedIncomeForMonth` upsert / clear-with-null / single-doc invariant / per-month independence / negative-rejection / activity-log emission; `monthly_occurrences_override` wins over interval-based math in `listOutsourceable`
- **Cadence-shift wiring** (§50 Phase E) — `patchRoutine` allow-list now exposes `outsourceable` / `outsource_cost_estimate` / `monthly_occurrences_override`; `skip_one` strategy clears `appointment.calendar_event_id` + `last_event_start` while preserving `appointment.enabled`
