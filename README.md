...
> **§50 rebuild status — Phases A + B + C shipped 2026-05-11.** The system has been re-shaped from a six-tab "Household Ops planner" into a three-view interface (Today / Look Back / Stuff) with one unified Claude assistant in place of the three-persona launcher pattern. **Phases D-G remain.** Phase D fills in Look Back's patterns. Phase E refines Stuff (cadence-shift modal, schema simplification). **Phase F is the most-urgent remaining work — the Alexa skill currently 404s against the trimmed API.** Phase G is final polish.
...
```bash
npm test                 # all workspaces — currently 199 tests (189 API + 10 alexa-skill)
